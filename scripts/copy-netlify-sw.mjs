...
// netlify/sappers/ (craigmcn.com/sappers) copy never gets one from the build
// itself. Everything else in that directory is already byte-identical
// between the two outputs (same base: './'), so copying the service worker
// files across is sufficient rather than building twice.
import { copyFileSync, readdirSync } from "node:fs";
import { join } from "node:path";

const SRC_DIR = "netlify";
const DEST_DIR = "netlify/sappers";
